...
//  ISC BOOK
//
//  Created by Gomi Kouki on 2023/12/30.
//
import UIKit
import Photos
import AVFoundation
...
class BarcodeViewController: UIViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "bookLend" {
            ...
            VC.BookTitleText = BookTitle
        }
    }
    
    func searchBook(){
        db.collection("Book").whereField("isbn", isEqualTo: codeLabel.text!).getDocuments { QuerySnapshot, Error in
            if let Error = Error{
                print(Error)
                return
            }
            ...
            guard let documents = QuerySnapshot?.documents, !documents.isEmpty else {
                let alert = UIAlertController(title: "該当するISBNコードが見つかりませんでした。",message: "読み取られたISBNコードに該当する本が見つかりませんでした。確認の上、もう一度お試しください。", preferredStyle: .alert)
            ...
            for document in QuerySnapshot!.documents{
                ...
                self.getBookTitle(documentID: documentID)
            }
        }
        
    }
    
    func getBookTitle(documentID: String){
